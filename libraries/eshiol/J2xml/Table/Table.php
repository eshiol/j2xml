...
class Table extends \JTable
{
		...
		if ($ret = parent::load($keys, $reset)) {
			...
			if (isset($this->access)) {
				...
				if ($serverType === 'postgresql') {
					$query->select('CASE WHEN ' . $this->_db->quoteName('v.id') . '<=6 THEN TO_CHAR(' . $this->_db->quoteName('v.id') . ', \'9\') ELSE ' . $this->_db->quoteName('v.title') . ' END');
				} else {
					$query->select('IF(' . $this->_db->quoteName('v.id') . '<=6, ' . $this->_db->quoteName('v.id') . ', ' . $this->_db->quoteName('v.title') . ')');
				}
				$query->from($this->_db->quoteName('#__viewlevels', 'v'))
					->join('RIGHT', $this->_db->quoteName($this->_tbl, 'a') . ' ON ' . $this->_db->quoteName('v.id') . ' = ' . $this->_db->quoteName('a.access'))
	...
	public static function getArticleId($path)
	{
		...
		$db = \JFactory::getDBO();
		$i = strrpos($path, '/');
		$article_id = $db->setQuery($db->getQuery(true)
	...
	public static function getAccessId($access)
	{
		...
		$db = \JFactory::getDBO();
		$access_id = $db->setQuery($db->getQuery(true)
			->select($db->quoteName('id'))
	...
	public static function getCategoryId($category, $extension, $default_category = 0)
	{
		...
		$db = \JFactory::getDBO();
		$category_id = $db->setQuery($db->getQuery(true)
			->select($db->quoteName('id'))
	...
	public static function getTagId($tag)
	{
		...
		try {
			if (is_array($tag)) {
				...
				$query = 'SELECT CASE WHEN b.id IS NOT NULL THEN b.id ELSE CONCAT(\'#new#\', a.path) END FROM (' . 'SELECT ' . $db->quote(array_shift($tags)) . ' as path';
				foreach ($tags as $tag) {
	...
	protected static function fixDate($date)
	{
		...
		return (($date == '0000-00-00 00:00:00') || ($date == '1970-01-01 00:00:00')) ? \JFactory::getDbo()->getNullDate() : (new \JDate($date))->toSQL(false);
	}
